...
template <int p> struct MomentumInteriorOperator {
    ...
    bool variableProp = true) : variableProp_(variableProp)
  {
    ...
    dnv_ = gather_field<p>(bulk, selector, dnvField);
  }
  ...
  void initialize(const stk::mesh::BulkData& bulk, stk::mesh::Selector selector)
  {
    ...
    const auto& viscField = *meta.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "effective_viscosity_u");
    effvisc_ = gather_field<p>(bulk, selector, viscField.field_of_state(stk::mesh::StateNone));
...
//      effvisc_ = nodal_wale_viscosity<p>(0.325, coords_, rhop1_, lamvisc_, dnv_, velp1_);
//    }

    scaled_volume_ = volumes<p>(rhop1_, coords_);
    mapped_area_ = mapped_areas<p>(effvisc_, coords_);
  }
  ...
  template <typename VectorViewType> void compute_rhs(
    ...
    VectorViewType yshared) const
  {
    ...
    Kokkos::parallel_for("momentum_rhs", range, KOKKOS_LAMBDA(const int index) {
      add_element_rhs_to_local_tpetra_vector<p>(
        index,
        maxOwnedRowLid,
        maxSharedNotOwnedLid,
        entityOffsets_,
        op::residual(index, gamma_, mdot_, volume_, coords_, effvisc_,
    ...
    VectorViewType yshared) const
  {
    ...
    if (xv.extent_int(1) == 3) {
      Kokkos::parallel_for("momentum_vector_residual", range, KOKKOS_LAMBDA(const int index) {
        ...
        add_element_rhs_to_local_tpetra_vector<p>(
          ...
          maxSharedNotOwnedLid,
          entityOffsets_,
          op::linearized_residual(index, gamma_[0], scaled_volume_, mapped_area_, mdot_, delta_view),
          yowned,
          yshared
        );
      });
      ...
      Kokkos::parallel_for("momentum_residual", range, KOKKOS_LAMBDA(const int index) {
  ...
  void set_mdot(elem_view::scs_scalar_view<p> mdot) { mdot_ =  mdot; }

  elem_view::vector_view<p> predict_state(const stk::mesh::BulkData& bulk, const stk::mesh::Selector& selector)
  ...
  elem_view::scalar_view<p> scaled_volume_;
  elem_view::scs_vector_view<p> mapped_area_;
  elem_view::scs_scalar_view<p> mdot_;

  //temp
  elem_view::scalar_view<p> pressure_;
  elem_view::scalar_view<p> dnv_;

  elem_view::scalar_view<p> effvisc_;

  elem_view::scalar_view<p> rhom1_;
  elem_view::scalar_view<p> rhop0_;
  elem_view::scalar_view<p> rhop1_;
  elem_view::vector_view<p> Gp_;
  elem_view::vector_view<p> velm1_;
  elem_view::vector_view<p> velp0_;
