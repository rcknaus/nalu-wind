...
void momentum_body_force_rhs(
  const CVFEMOperators<p, Scalar>& ops,
  const nodal_scalar_view<p, Scalar>& vol,
  const Kokkos::Array<double, 3>& fb,
  nodal_vector_view<p, Scalar>& rhs)
{
  constexpr int n1D = p + 1;

  nodal_vector_array<Scalar, p> work_drhoudt;
  for (int k = 0; k < n1D; ++k) {
    for (int j = 0; j < n1D; ++j) {
      for (int i = 0; i < n1D; ++i) {
        const Scalar local_vol = vol(k, j, i);
        work_drhoudt(k, j, i, XH) = fb[XH] * local_vol;
        work_drhoudt(k, j, i, YH) = fb[YH] * local_vol;
        work_drhoudt(k, j, i, ZH) = fb[ZH] * local_vol;
      }
    }
  }
  auto drhoudt = la::make_view(work_drhoudt);
