...
{
  static constexpr int n1D = p + 1;
  using type = Kokkos::View<DoubleType*[n1D][n1D],
      typename ViewTraits<ExecSpace>::layout,
      typename ViewTraits<ExecSpace>::memory_space,
      typename ViewTraits<ExecSpace>::memory_traits>;
};
...
{
  static constexpr int n1D = p + 1;
  using type = Kokkos::View<DoubleType*[n1D][n1D][3],
      typename ViewTraits<ExecSpace>::layout,
      typename ViewTraits<ExecSpace>::memory_space,
      typename ViewTraits<ExecSpace>::memory_traits>;
};
...
template <int p, typename ExecSpace> struct GlobalArrayTypeSelector<p, FieldType::SCS_SCALAR, ExecSpace>
{
  ...
  using type = Kokkos::View<DoubleType*[2][n1D][n1D],
      typename ViewTraits<ExecSpace>::layout,
      typename ViewTraits<ExecSpace>::memory_space,
      typename ViewTraits<ExecSpace>::memory_traits>;
};
...
{
  static constexpr int n1D = p + 1;
  using type = Kokkos::View<DoubleType*[2][n1D][n1D][3],
      typename ViewTraits<ExecSpace>::layout,
      typename ViewTraits<ExecSpace>::memory_space,
      typename ViewTraits<ExecSpace>::memory_traits>;
};
...
namespace node_view {
  ...
  template <FieldType, typename ExecSpace> struct GlobalArrayTypeSelector {};

  template <typename ExecSpace> struct GlobalArrayTypeSelector<FieldType::SCALAR, ExecSpace>
  {
    using type = Kokkos::View<double*,
        typename ViewTraits<ExecSpace>::layout,
        typename ViewTraits<ExecSpace>::memory_space,
        typename ViewTraits<ExecSpace>::memory_traits>;
  };

  template <typename ExecSpace> struct GlobalArrayTypeSelector<FieldType::VECTOR, ExecSpace>
  {
    using type = Kokkos::View<double*[3],
        typename ViewTraits<ExecSpace>::layout,
        typename ViewTraits<ExecSpace>::memory_space,
        typename ViewTraits<ExecSpace>::memory_traits>;
  };
  ...
  using scalar_view = typename GlobalArrayTypeSelector<FieldType::SCALAR, ExecSpace>::type;
  template <typename ExecSpace = Kokkos::DefaultExecutionSpace>
  using vector_view = typename GlobalArrayTypeSelector<FieldType::VECTOR, ExecSpace>::type;
}
}}

namespace sierra { namespace nalu {

using node_offset_view_t = Kokkos::View<int*>;

template <int p> struct face_ordinal_view
{
  static constexpr int n1D = p + 1;
  using type = Kokkos::View<int*[simdLen][n1D][n1D]>;
};
template <int p> using face_ordinal_view_t = typename face_ordinal_view<p>::type;
...
template <int p, typename Scalar = DoubleType> struct CVFEMViews {
  ...
  using nodal_coefficient_matrix = LocalArray<Scalar[n1D][n1D]>;

  using face_nodal_scalar_array = LocalArray<Scalar[n1D][n1D]>;
  using face_nodal_scalar_view = la::view_type<face_nodal_scalar_array>;

  using face_nodal_vector_array = LocalArray<Scalar[n1D][n1D][dim]>;
  using face_nodal_vector_view = la::view_type<face_nodal_vector_array>;

  using face_nodal_tensor_array = LocalArray<Scalar[n1D][n1D][dim][dim]>;
  using face_nodal_tensor_view = la::view_type<face_nodal_tensor_array>;

  using face_scs_scalar_array = LocalArray<Scalar[dim][n1D][n1D]>;
  using face_scs_scalar_view = la::view_type<face_scs_scalar_array>;

  using face_scs_vector_array =  LocalArray<Scalar[dim][n1D][n1D][dim]>;
  using face_scs_vector_view = la::view_type<face_scs_vector_array>;
};
...
template <typename Scalar, int p> using face_nodal_scalar_array = typename CVFEMViews<p,Scalar>::face_nodal_scalar_array;
template <typename Scalar, int p> using face_nodal_vector_array = typename CVFEMViews<p,Scalar>::face_nodal_vector_array;
template <typename Scalar, int p> using face_nodal_tensor_array = typename CVFEMViews<p,Scalar>::face_nodal_tensor_array;
template <typename Scalar, int p> using face_scs_scalar_array = typename CVFEMViews<p,Scalar>::face_scs_scalar_array;
template <typename Scalar, int p> using face_scs_vector_array = typename CVFEMViews<p,Scalar>::face_scs_vector_array;

template <typename Scalar, int p> using face_nodal_scalar_view = typename CVFEMViews<p,Scalar>::face_nodal_scalar_view;
template <typename Scalar, int p> using face_nodal_vector_view = typename CVFEMViews<p,Scalar>::face_nodal_vector_view;
template <typename Scalar, int p> using face_nodal_tensor_view = typename CVFEMViews<p,Scalar>::face_nodal_tensor_view;
template <typename Scalar, int p> using face_scs_scalar_view = typename CVFEMViews<p,Scalar>::face_scs_scalar_view;
template <typename Scalar, int p> using face_scs_vector_view = typename CVFEMViews<p,Scalar>::face_scs_vector_view;
