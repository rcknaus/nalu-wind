  ...
  ScratchViews<DoubleType>& scratchViews)
{
  ...
  for (int ip=0; ip < AlgTraits::numScvIp_; ++ip) {
    ...
    rhs(offset + 0) -= scV * (-(M_PI*(1 + stk::math::cos(4*M_PI*y) - 2*stk::math::cos(4*M_PI*z))*stk::math::sin(4*M_PI*x))/8.
        + 6*mu_*(M_PI * M_PI)*stk::math::cos(2*M_PI*x)*stk::math::sin(2*M_PI*y)*stk::math::sin(2*M_PI*z));

    rhs(offset + 1) -= scV * (-(M_PI*(1 + stk::math::cos(4*M_PI*y) - 2*stk::math::cos(4*M_PI*z))*stk::math::sin(4*M_PI*x))/8.
        + 6*mu_*(M_PI * M_PI)*stk::math::cos(2*M_PI*x)*stk::math::sin(2*M_PI*y)*stk::math::sin(2*M_PI*z));

    rhs(offset + 2) -= scV * (M_PI*(24*mu_*M_PI*stk::math::cos(2*M_PI*z)*stk::math::sin(2*M_PI*x)*stk::math::sin(2*M_PI*y)
