...
template <int p> LocalArray<DoubleType[p][p][p][8][8]> sparsified_laplacian_lhs(const nodal_vector_view<p, DoubleType>& local_coords)
{
  ...
  static const auto ops = CVFEMOperators<1, DoubleType>();
  for (int n = 0; n < p; ++n) {
    for (int m = 0; m < p; ++m) {
      for (int l = 0; l < p; ++l) {
        NALU_ALIGNED DoubleType box[3][8];
        hex_vertex_coordinates(n, m, l, local_coords, box);
        auto metric = high_order_metrics::compute_laplacian_metric_box<DoubleType>(box);
        auto v_metric = la::make_view(metric);

        auto lhs = la::zero<matrix_array<DoubleType, 1>>();
        auto v_lhs = la::make_view(lhs);
        tensor_assembly::scalar_diffusion_lhs<1, DoubleType>(ops, v_metric, v_lhs);

        for (int j = 0; j < 8; ++j) {
          for (int i = 0; i < 8; ++i) {
            sparsified_lhs(n,m,l,j,i) = lhs(j, i);
          }
        }
      }
    }
...
template LocalArray<DoubleType[POLY1][POLY1][POLY1][8][8]> sparsified_laplacian_lhs(const nodal_vector_view<POLY1, DoubleType>&);
template LocalArray<DoubleType[POLY2][POLY2][POLY2][8][8]> sparsified_laplacian_lhs(const nodal_vector_view<POLY2, DoubleType>&);
template LocalArray<DoubleType[POLY3][POLY3][POLY3][8][8]> sparsified_laplacian_lhs(const nodal_vector_view<POLY3, DoubleType>&);
template LocalArray<DoubleType[POLY4][POLY4][POLY4][8][8]> sparsified_laplacian_lhs(const nodal_vector_view<POLY4, DoubleType>&);

template <int p> LocalArray<DoubleType[p][p][p][12][2][2]> sparsified_laplacian_edge_lhs(const nodal_vector_view<p, DoubleType>& local_coords)
{
  auto sparsified_lhs = la::zero<LocalArray<DoubleType[p][p][p][12][2][2]>>();
      ...
      for (int l = 0; l < p; ++l) {
        ...
        for (int k = 0; k < 2; ++k) {
          ...
          for (int j = 0; j < 2; ++j) {
            ...
            sparsified_lhs(n, m, l, edge_ord, 0, 0) += dfdq;
            sparsified_lhs(n, m, l, edge_ord, 0, 1) -= dfdq;
            sparsified_lhs(n, m, l, edge_ord, 1, 0) -= dfdq;
            sparsified_lhs(n, m, l, edge_ord, 1, 1) += dfdq;
            edge_ord++;
          }
        }
...
template LocalArray<DoubleType[POLY1][POLY1][POLY1][12][2][2]> sparsified_laplacian_edge_lhs(const nodal_vector_view<POLY1, DoubleType>&);
template LocalArray<DoubleType[POLY2][POLY2][POLY2][12][2][2]> sparsified_laplacian_edge_lhs(const nodal_vector_view<POLY2, DoubleType>&);
template LocalArray<DoubleType[POLY3][POLY3][POLY3][12][2][2]> sparsified_laplacian_edge_lhs(const nodal_vector_view<POLY3, DoubleType>&);
template LocalArray<DoubleType[POLY4][POLY4][POLY4][12][2][2]> sparsified_laplacian_edge_lhs(const nodal_vector_view<POLY4, DoubleType>&);
