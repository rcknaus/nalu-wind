...
MomentumBuoyancyBoussinesqSrcHOElemKernel<AlgTraits>::MomentumBuoyancyBoussinesqSrcHOElemKernel(
    ...
    beta_(solnOpts.thermalExpansionCoeff_)
{
  ...
  temperatureField_ = &meta_data.get_field<ScalarFieldType>(stk::topology::NODE_RANK, "temperature")
    ->field_of_state(stk::mesh::StateNew);
  dataPreReqs.add_gathered_nodal_field(*temperatureField_, 1);

}
//--------------------------------------------------------------------------
template <class AlgTraits> void
MomentumBuoyancyBoussinesqSrcHOElemKernel<AlgTraits>::execute(
  ...
  ScratchViewsHO<DoubleType>& scratchViews)
{
  ...
  const auto temperature = scratchViews.get_scratch_view<nodal_scalar_view>(*temperatureField_);
  ...
  for (int k = 0; k < AlgTraits::nodes1D_; ++k) {
    for (int j = 0; j < AlgTraits::nodes1D_; ++j) {
      for (int i = 0; i < AlgTraits::nodes1D_; ++i) {
        const DoubleType fac = -rhoRef_ * beta_ * (temperature(k, j, i) - tRef_) * vol(k, j, i);
        for (int d = 0; d < 3; ++d) {
          buoyancy_src(k, j, i, d) = gravity_[d] * fac;
        }
      }
    }
  }
...
INSTANTIATE_KERNEL_HOSGL(MomentumBuoyancyBoussinesqSrcHOElemKernel)
