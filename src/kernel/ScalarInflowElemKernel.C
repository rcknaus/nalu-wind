...
ScalarInflowElemKernel<BcAlgTraits>::ScalarInflowElemKernel(
  ...
  : Kernel(),
    skewSymmetric_(solnOpts.get_skew_symmetric(q.name())),
    projTimeScale_(1.0),
    ipNodeMap_(MasterElementRepo::get_surface_master_element(BcAlgTraits::faceTopo_)->ipNodeMap())
 {
  ...
  velocityBC_ = meta.get_field<VectorFieldType>(stk::topology::NODE_RANK, "velocity_bc");
  faceDataPreReqs.add_gathered_nodal_field(*velocityBC_, dim);


  q_ = &q.field_of_state(stk::mesh::StateNP1);
  faceDataPreReqs.add_gathered_nodal_field(*q_, 1);
  elemDataPreReqs.add_gathered_nodal_field(*q_, 1);

  qBC_ = &qBC.field_of_state(stk::mesh::StateNone);
  faceDataPreReqs.add_gathered_nodal_field(*qBC_, 1);
  ...
  diff_ = &diffusivity.field_of_state(stk::mesh::StateNone);
  faceDataPreReqs.add_gathered_nodal_field(*diff_, 1);
  elemDataPreReqs.add_gathered_nodal_field(*diff_, 1);
...
template<typename BcAlgTraits>
void
ScalarInflowElemKernel<BcAlgTraits>::execute(
  ...
  int elemFaceOrdinal)
{
  ...
  SharedMemView<DoubleType*>& face_mu = faceViews.get_scratch_view_1D(*diff_);
  SharedMemView<DoubleType*>& face_q = faceViews.get_scratch_view_1D(*q_);
  SharedMemView<DoubleType*>& face_qBC = faceViews.get_scratch_view_1D(*qBC_);
  ...
  SharedMemView<DoubleType*>& elem_q = elemViews.get_scratch_view_1D(*q_);
  ...
  for (int ip = 0; ip < BcAlgTraits::numFaceIp_; ++ip) {
    ...
    for (int n = 0; n <  BcAlgTraits::nodesPerFace_; ++n) {
      lhs(nn, face_node_ordinals[n]) += vf_adv_shape_function_(ip, n) * mdotBCIp;
    }
    rhs(nn) -= mdotBCIp * qIp;

    for (int n = 0; n < BcAlgTraits::nodesPerFace_; ++n) {
      lhs(nn, face_node_ordinals[n]) += -vf_adv_shape_function_(ip, n) * penaltyFac;
    }
    rhs(nn) -= -penaltyFac * (qIp - qBCIp);

    NALU_ALIGNED Kokkos::Array<DoubleType, 9> viscFluxIp = {{0,0,0}};
    for ( int n = 0; n < BcAlgTraits::nodesPerElement_; ++n ) {
      for ( int j = 0; j < dim; ++j ) {
        viscFluxIp[j] += viscIp * v_dndx(ip, n, j) * elem_q(n);
      }
    }

    for (int n = 0; n < BcAlgTraits::nodesPerElement_; ++n) {
      lhs(nn, n) += -viscIp * ddot(&v_dndx(ip, n, 0), areavIp, dim);
    }
    rhs(nn) -= -ddot(&viscFluxIp[0], areavIp, dim);
  }
}

INSTANTIATE_KERNEL_FACE_ELEMENT(ScalarInflowElemKernel)
