...
double MatrixFreeProblem::residual_norm()
{
  Kokkos::View<double*> mv_norm("norms", rhs->getNumVectors());
  rhs->norm2(mv_norm);

  double norm = 0;
  for (unsigned d = 0; d < rhs->getNumVectors(); ++d) {
    norm += mv_norm(d) * mv_norm(d);
  }
  return sqrt(norm);
}
...
TpetraMatrixFreeSolver::TpetraMatrixFreeSolver(int ndim) : ndim_(ndim)
{
  ...
  precondParams->set("relaxation: use l1", true);
...
void TpetraMatrixFreeSolver::solve()
{
  ...
  if (print && prob_->getRHS()->getNumVectors() == 3) {
    ...
    for (int k = 0; k < y_view.extent_int(0); ++k) {
      std::cout << k << "(x,y): (" << x_view(k,0) << ", "
          << y_view(k,0)  << ", " << y_view(k,1) << ", "  << y_view(k,2)<<")" << std::endl;
    }
