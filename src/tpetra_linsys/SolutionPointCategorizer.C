...
namespace sierra {
namespace nalu {
//  if (entityIsOwned && entityIsShared) return (SolutionPointStatus::owned & SolutionPointStatus::shared);
//  if (!entityIsOwned && entityIsShared) return (~SolutionPointStatus::owned & SolutionPointStatus::shared);
//  if (entityIsOwned && !entityIsShared) return (SolutionPointStatus::owned & ~SolutionPointStatus::shared);
//  if (!entityIsOwned && !entityIsShared) return SolutionPointStatus::ghosted;
...
{
  //  rule: slave if global id has been overwritten by the master node GID
...
{
  if (!is_slave(node)) return regular_status(node);
