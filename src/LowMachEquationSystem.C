  ...
  const WallBoundaryConditionData &wallBCData)
{
  ...
  if (anyWallFunctionActivated) {
   ...
   if (ablWallFunctionApproach) {
    ...
    const AlgorithmType wfAlgType =
      (ablWallFunctionApproach) ? WALL_ABL : WALL_FCN;
   ...
   if (ablWallFunctionApproach) {
     ...
     wallFuncAlgDriver_.register_face_elem_algorithm<SmoothWallFluxesAlg>(
       wfAlgType, part, get_elem_topo(realm_, *part), "smooth_wall_func",
       wallFuncAlgDriver_);
    }
  ...
  if (RANSAblBcApproach) {
    ...
    if (it_wf == solverAlgDriver_->solverAlgMap_.end()) {
      solverAlgDriver_->solverAlgMap_[wfAlgType] =
        new AssembleMomentumEdgeWallFunctionSolverAlgorithm(realm_, part, this);
    }
    else {
