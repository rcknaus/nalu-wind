...
public:
  ...
  void create()
  {
    ...
    if (precondition) {
      mfDiag_ = make_rcp<MomentumInteriorDiagonal<MF::p>>(bulk, selector, tpetralinsys, *eqSys_.coordinates_);
  ...
  Teuchos::RCP<MomentumInteriorOperator<MF::p>> mfInterior_;
  Teuchos::RCP<NoOperator> mfBdry_;
  Teuchos::RCP<MomentumInteriorDiagonal<MF::p>> mfDiag_;
...
public:
  ...
  void create()
  {
    ...
    mfInterior_ = make_rcp<ContinuityInteriorOperator<MF::p>>(bulk, selector, coordField, entToLid);
    mfBdry_ = make_rcp<NoOperator>();
    ...
    if (precondition) {
      mfSparse_ = make_rcp<SparsifiedLaplacianInterior<MF::p>>(bulk, selector, tpetralinsys, coordField);
      tpetralinsys.zeroSystem();
  ...
  Teuchos::RCP<ContinuityInteriorOperator<MF::p>> mfInterior_;
  Teuchos::RCP<NoOperator> mfBdry_;
  Teuchos::RCP<SparsifiedLaplacianInterior<MF::p>> mfSparse_;
  Teuchos::RCP<MatrixFreeProblem> mfProb_;
  Teuchos::RCP<mfop_type> mfOp_;
  Teuchos::RCP<TpetraMatrixFreeSolver> solver_;
};
...
ContinuityEquationSystem::initialize()
{
  if (MF::doMatrixFree) {
    ...
    auto& tpetralinsys = *dynamic_cast<TpetraLinearSystem*>(linsys_);
    tpetralinsys.buildSparsifiedElemToNodeGraph(stk::mesh::selectUnion(realm_.interiorPartVec_));
//    tpetralinsys.buildElemToNodeGraph(realm_.interiorPartVec_);
